...
class HtmlUtils {
  static generateCode2HTml(String mdData,
      {String backgroundColor = SummerColor.miWhiteString,
      String lang = 'java',
      userBR = true}) {
  ...
  static generateHtml(String mdData,
      {String backgroundColor = SummerColor.webDraculaBackgroundColorString,
      userBR = true}) {
    if (mdData == null) {
      return "";
    }
    ...
    return generateCodeHtml(mdDataCode, false,
        ...
        actionColor: SummerColor.actionBlueString,
        userBR: userBR);
  }

  ///style for mdHTml
  static generateCodeHtml(mdHTML, wrap,
      {backgroundColor = SummerColor.white,
      String actionColor = SummerColor.actionBlueString,
      userBR = true}) {
    return "<html>\n" +
        "<head>\n" +
        "<meta charset=\"utf-8\" />\n" +
        ...
        "tr:nth-child(even) {" +
        "background:" +
        SummerColor.primaryLightValueString +
        ";" +
        "color:" +
        SummerColor.miWhiteString +
        ";" +
  ...
  static resolveHtmlFile(var res, String defaultLang) {
    if (res != null && res.result) {
      ...
      if ('markdown' == lang) {
        return generateHtml(res.data, backgroundColor: SummerColor.miWhiteString);
      } else {
        return generateCode2HTml(res.data,
            backgroundColor: SummerColor.webDraculaBackgroundColorString,
            lang: lang);
      }
    } else {
